...
$(document).ready(function(){
	...
	**/
	...
	$.get("update/check?rand=" + new Date(),function(result){
		if(result!="0"){
			if(result.indexOf("<!--isAutoUpdate-->")>-1){
				$.get("update/installexec?rand=" + new Date());
			}else{
				if(result.indexOf("<!--mustInstall-->")>-1){
					$.dialog({
						title:	"必须安装的更新包",
						width:	'500px',
						height:	'120px',
	    				...
	    				resize: false,
	    				fixed: true,
	    				content: "<div id='updatePatchMustPop'>"+ result +"</div>",
				...
				$("#updatePatchPop a,#updatePatchMustPop a").click(function(){
	...
	$.get("updatePlugin/check?rand=" + new Date(),function(result){
		if(result!="0"){
			$("#updatePluginPatchPop").html(result);
			$("#updatePluginPatchPop #pluginUpdateShow").click(function(){
				var mydialog = $.dialog.open("updatePlugin/detail?rand=" + new Date(),{
				title :	'查看更新',
				width : '550px',
				height:	'350px',
				lock  :	true,
	...
	/*window.setInterval(function(){
		$.get("/updateSession?rand=" + new Date());
	},540000);*/
